...
class Minesweeper:
    ...
    def tampilkan(self):
        ...
        print("  " + "---" + "---" * (self.lebar - 1))  # Menambahkan garis pemisah
        for y in range(self.tinggi):
            print(str(y) + "|", end=" ")  # Menampilkan nomor baris
            for x in range(self.lebar):
                if self.permainan_selesai and self.papan[y][x] == '*':
                    print('x' + "|", end=" ")  # Menampilkan ranjau sebagai 'x' jika permainan selesai
                else:
                    print(self.terlihat[y][x] + "|", end=" ")  # Menampilkan isi sel
